...
class SkyfishSearchWidget extends WidgetBase {
   ...
   *   Saved file details.
   */
  protected function downloadSkyfishFile(stdClass $item) {
  ...
  protected function getKeyedFolders(): array {
    ...
    if (count($keyed_folders) === 0) {
      Drupal::messenger()->addError('Check Skyfish settings: admin/config/media/media_skyfish: no folders found.');
    }
    return $keyed_folders;
  }
